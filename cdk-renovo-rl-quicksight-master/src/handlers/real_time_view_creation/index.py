...
def handler(event, context):
    ...
    try:
        ...
        if cdc_table_exist:
            ...
            dynamic_query = f"""
            CREATE OR REPLACE VIEW {rl_database}.v_{table} AS
            WITH snapshot_max AS (
                SELECT MAX(ingestion_timestamp) AS max_ts
                FROM {rl_database}.{table}_snapshot
            ),
            snapshot_max_parts AS (
                SELECT
            ...
            cdc_data AS (
                ...
                WHERE ingestion_timestamp > (SELECT max_ts FROM snapshot_max)
                    AND (
                        (CAST(cdc.partition_0 AS int) = (SELECT yr FROM snapshot_max_parts) AND
                        CAST(cdc.partition_1 AS int) = (SELECT mo FROM snapshot_max_parts) AND
                        CAST(cdc.partition_2 AS int) >= (SELECT dy FROM snapshot_max_parts))
                        OR
                        (CAST(cdc.partition_0 AS int) = (SELECT yr FROM snapshot_max_parts) AND
                        CAST(cdc.partition_1 AS int) > (SELECT mo FROM snapshot_max_parts))
                        OR
                        (CAST(cdc.partition_0 AS int) > (SELECT yr FROM snapshot_max_parts))
                    )
            ),
            combined AS (
                SELECT
            ...
            SELECT
                ...
                *
            FROM ranked
            WHERE rn = 1 AND op != 'D' WITH NO SCHEMA BINDING;
            """
        else:
            ...
            FROM {rl_database}.{table}_snapshot WITH NO SCHEMA BINDING;
            """
        ...
        if status == "FINISHED":
            ...
            failure_details = redshift_data_client.describe_statement(Id=query_id)
            logger.error(f"Error occurred while creating or replacing the view: {failure_details.get('Error')}")
            raise Exception(f"Error occurred while creating or replacing the view: {failure_details.get('Error')}")
    ...
    return {
        'statusCode': 200,
        'body': 'View created or replaced successfully.',
    }
