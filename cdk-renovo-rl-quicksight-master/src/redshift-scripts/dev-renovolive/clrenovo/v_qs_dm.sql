CREATE OR REPLACE VIEW clrenovo.v_qs_dm AS
WITH filtered_se_data AS (
	SELECT * FROM clrenovo.v_se_serviceevents
...
SELECT
    ...
    COALESCE(clrenovo.v_dim_total_parts_used_by_se.parts_used, 0) AS parts_used,
...
LEFT JOIN clrenovo.v_dim_po_numbers_by_se AS po
    ON sev.serviceEventID = po.ServiceEventID
LEFT JOIN clrenovo.v_facility_equipment AS feq
...
LEFT JOIN clrenovo.v_mtcldeviceTemps AS emdt
    ON feq.mtclDeviceTempID = emdt.mtcldevicetempid
LEFT JOIN clrenovo.v_facility_costCenters AS efcc
    ON feq.facility_costCenterID = efcc.facility_costCenterID
LEFT JOIN clrenovo.v_facilities AS efac
    ON efcc.facilityid = efac.facilityid
LEFT JOIN clrenovo.v_clientSystems AS ecs
    ON efac.systemid = ecs.systemid
LEFT JOIN clrenovo.v_clients AS eccs
    ON ecs.clientid = eccs.clientid
LEFT JOIN clrenovo.v_ismSystems AS isys
...
LEFT JOIN clrenovo.v_mtcldeviceTemps AS imdt
    ON isys.mtclDeviceTempID = imdt.mtcldevicetempid
LEFT JOIN clrenovo.v_facility_costCenters AS ifcc
    ON isys.facility_costCenterID = ifcc.facility_costCenterID
LEFT JOIN clrenovo.v_facilities AS ifac
    ON ifcc.facilityid = ifac.facilityid
LEFT JOIN clrenovo.v_clientSystems AS ics
    ON ifac.systemid = ics.systemid
LEFT JOIN clrenovo.v_clients AS iccs
    ON ics.clientid = iccs.clientid
LEFT JOIN clrenovo.v_facility_costcenters AS cfcc
    ON sev.facility_costcenterid = cfcc.facility_costcenterid
LEFT JOIN clrenovo.v_facilities AS cfac
    ON cfcc.facilityID = cfac.facilityID
LEFT JOIN clrenovo.v_clientSystems AS ccs
    ON cfac.systemid = ccs.systemid
LEFT JOIN clrenovo.v_clients AS cccs
    ON ccs.clientid = cccs.clientid
LEFT JOIN clrenovo.v_l_sestatus AS lss
    ON sev.l_sestatusid = lss.sestatusid
LEFT JOIN clrenovo.v_dim_se_assigned AS dim_se_assigned
    ON dim_se_assigned.serviceEventID = sev.serviceEventID
LEFT JOIN clrenovo.v_se_specialConditions AS sesc
...
LEFT JOIN RenovoMaster.v_l_lookups AS ll
    ...
    AND ll.parentID = 34
LEFT JOIN clrenovo.v_l_seSpecialConditions AS lsc
    ON sesc.l_seSpecialConditionID = lsc.seSpecCondID
LEFT JOIN clrenovo.v_dim_se_notes AS notes_cte
    ON sev.serviceEventID = notes_cte.serviceEventID
LEFT JOIN clrenovo.v_dim_total_parts_used_by_se
    ON sev.serviceEventID = clrenovo.v_dim_total_parts_used_by_se.serviceEventID
LEFT JOIN clrenovo.v_l_seResolutions AS lrs
    ON sev.l_seResolutionID = lrs.seResID
LEFT JOIN clrenovo.v_l_seSymptoms AS lsm
    ON sev.l_seSymptomID = lsm.seSymptID
LEFT JOIN clrenovo.v_l_seDiagnosis AS ldm
    ON sev.l_seDiagnosisID = ldm.seDiagnID
LEFT JOIN clrenovo.v_dim_total_parts_ordered_by_se AS cpart
